  ...
  const clearData = () => {
    Alert.alert("Очистить локальные данные?", "Будут удалены история чата, список материалов, настройки и локальные копии добавленных файлов. Это действие нельзя отменить.", [
      { text: "Отмена", style: "cancel" },
      {
        text: "Очистить",
        style: "destructive",
        onPress: () => { void clearWorkspaceData().then(() => router.back()); },
      },
    ]);
  };
    ...
    <ScreenContainer className="px-4" edges={["top", "bottom", "left", "right"]} containerClassName="bg-background">
      <ScrollView contentContainerStyle={styles.content} showsVerticalScrollIndicator={false}>
          ...
          <Ai40Card style={styles.group}>
            <View style={styles.settingRow}>
              <View style={styles.settingCopy}>
                <Text style={styles.settingTitle}>Передавать выбранный контекст</Text>
                <Text style={styles.settingText}>Когда включено, текст материалов, которые вы отметили в библиотеке, добавляется к следующему сообщению ассистенту.</Text>
              </View>
              <Switch value={settings.sendSelectedContext} onValueChange={(value) => { void update({ ...settings, sendSelectedContext: value }); }} trackColor={{ false: "#D9DDE9", true: "#B8B5FF" }} thumbColor={settings.sendSelectedContext ? palette.indigo : "#FFFFFF"} />
            </View>
            <View style={styles.divider} />
            <View style={styles.settingRow}>
              <View style={styles.settingCopy}>
                <Text style={styles.settingTitle}>Компактные ответы</Text>
                <Text style={styles.settingText}>Сохраняет это предпочтение для следующих обновлений интерфейса и режимов ответа.</Text>
              </View>
              <Switch value={settings.compactReplies} onValueChange={(value) => { void update({ ...settings, compactReplies: value }); }} trackColor={{ false: "#D9DDE9", true: "#B8B5FF" }} thumbColor={settings.compactReplies ? palette.indigo : "#FFFFFF"} />
            </View>
          </Ai40Card>
        </View>
        ...
        <View>
          ...
          <Ai40Card style={styles.dangerCard}>
            <Text style={styles.infoTitle}>Очистка рабочего пространства</Text>
            <Text style={styles.infoText}>Удаляет историю, настройки и локальные копии импортированных материалов. Не затрагивает исходные файлы за пределами приложения.</Text>
            <PrimaryButton label="Очистить данные" icon="delete-outline" tone="danger" onPress={clearData} />
          </Ai40Card>
        </View>
      </ScrollView>
    </ScreenContainer>
...
const styles = StyleSheet.create({
  ...
  header: { gap: 9 },
  lead: { color: palette.muted, fontSize: 13, lineHeight: 19 },
  group: { paddingVertical: 6 },
  settingRow: { flexDirection: "row", gap: 14, alignItems: "center", paddingVertical: 12 },
  settingCopy: { flex: 1, gap: 4 },
  settingTitle: { color: palette.ink, fontSize: 14, fontWeight: "800" },
  settingText: { color: palette.muted, fontSize: 12, lineHeight: 17 },
  ...
  infoText: { color: palette.muted, fontSize: 13, lineHeight: 19 },
