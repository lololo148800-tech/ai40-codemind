...
const BLOCKED_REQUESTS = [
  ...
  /(?:покажи|дай|вышли|раскрой).{0,80}(?:исход(?:ный)?\s*код|source\s*code|внутренн.{0,24}код|системн.{0,24}инструкц)/i,
];
...
export function blockedReason(text: string) {
  return BLOCKED_REQUESTS.some((pattern) => pattern.test(text))
    ? "Я не помогаю обходить защиту, использовать предполагаемые утечки, раскрывать внутренний исходный код или получать чужие секреты. Могу предложить официальный API, публичные источники или безопасную архитектуру."
    : null;
}
